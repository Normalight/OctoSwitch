...
- **Windows 更新后不重启**：修复下载并安装更新后应用关闭但不重新启动的问题。原因是 NSIS 安装程序需要关闭当前进程才能覆写 exe，但 `child.wait()` 造成死锁，导致重启代码无法执行。改用 helper 脚本等待进程退出后自动重启（仅影响 Windows）
...
- 启动失败时弹出本地化错误对话框（随系统语言中英文；可选环境变量 `OCTOSWITCH_UI_LANG` 覆盖）
- 供应商 / 模型 / 分组列表加载失败时展示可关闭的错误提示，不再静默为空列表
- 配置导入后仅依赖后端事件刷新数据，去掉重复的 `window` 派发，避免二次刷新

### Usage metrics

- KPI、趋势与快照中的「总消耗」计入 **`cache_creation_input_tokens`**，与请求日志字段一致

### Updates (Settings)

- 修复下载更新流程中界面状态误复位（移除不当超时、改进缓存初始化与下载中检查拦截）

### Rust

- 依赖：`rfd`（启动失败原生对话框）、`sys-locale`（语言判定）
...
- 新增自动检查更新开关与相关设置持久化
- 更新检查与下载体验优化，补充更完整的前端状态处理

### Usage metrics

- 用量统计的 Input 口径统一为“输入（含缓存）”
- KPI、趋势 tooltip、请求明细表头与数值展示保持一致
- 新增前端测试栈与用量统计组件测试，覆盖共享计算、KPI、趋势与明细
...
- 网关重启健康检查重试、更新启动顺序优化
- 刷新按钮形状、下载重检、降级弹窗、保存指示器修复
- 请求表格 Provider 列宽、Output i18n
- 趋势图 tooltip 优化
